...
pubDatetime: 2022-06-12T00:00:00Z
title: Randomised approach to matrix decomposition; Fast SVD
slug: randomised-approach-to-matrix-decomposition-fast-svd
featured: true
draft: false
tags:
  - deeplearning 🧠
...
- [Randomized Matrix Approximation](#randomized-matrix-approximation)
- [Need for a Randomized Approach](#need-for-a-randomized-approach)
...
Wall time: 3.48 s
```
Clearly the Randomized approach to SVD is much faster. Lets discuss the method and its implementation.
...
\begin{aligned}
A &\approx Q Q^T A \\
  &= Q (S\,\Sigma V^T) \quad \text{as} \quad B = Q^T A \\
  &= U \Sigma V^T \quad \text{if we construct} \quad U = QS
\end{aligned}
$$
...
[1] [Finding Structure with Randomness: Probabilistic Algorithms for Constructing Approximate Matrix Decompositions](https://arxiv.org/abs/0909.4061) is an excellent read.
